...
# --- 2. 核心下载引擎 ---
def downloader(ticker, start_date, end_date, method):
    """
    method: 1 为仅使用 yfinance, 2 为仅使用 Requests (API), 0 为混合模式
    """
    ...
    if method in [0, 1]:
        try:
            # 删除了 threading 参数以适配最新版 yfinance
            ...
            if method == 1: print(f"⚠️ yf 失败 [{ticker}]: {e}")
            ...
            if response.status_code == 200:
                ...
                if chart and "timestamp" in chart:
                    ...
                    if not df.empty:
                        ...
                        print(f"✅ Success (Requests API): {ticker}")
                        return True
        except Exception as e:
            print(f"⚠️ API 失败 [{ticker}]: {e}")
...
def download_main(market_option, method_option):
    ...
    mode_text = {1: "仅 yfinance", 2: "仅 Requests API", 0: "混合模式"}.get(method_option)
    print(f"🛠  当前下载模式: {mode_text}")
    ...
    for m_name in targets:
        try:
            ...
            for ticker in stocks:
                downloader(ticker, start_date, end_date, method_option)
                time.sleep(0.5) 
        except Exception as e:
...
if __name__ == '__main__':
    market_choice = 0  # 0:全部, 1:沪深, 2:标普, 3:加拿大
    method_choice = 2  # 1:仅 yfinance, 2:仅 Requests API, 0:混合模式

    download_main(market_choice, method_choice)
    print(f"\n🏁 同步结束: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
