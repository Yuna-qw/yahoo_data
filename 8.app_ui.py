...
【强制执行】：
1. 纯净输出：只输出 SQL，严禁任何解释性文字，严禁 Markdown 符号。
2. 语法规范：严禁反引号 `。严禁 DATE_SUB()。
3. 时间计算：减去时间使用：CURRENT_DATE - INTERVAL '6 MONTH'。
4. **时间过滤（核心）**：必须包含时间限制。针对 `stock_monthly_change` 用 `Month_Start_Date`，针对 `stock_data` 用 `Date`。
